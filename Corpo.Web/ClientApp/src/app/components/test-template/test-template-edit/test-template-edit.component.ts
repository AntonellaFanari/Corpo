...
export class TestTemplateEditComponent implements OnInit {
  ...
  addExercise() {
    ...
    if (this.formEdit.valid) {
      let exerciseRepeatedName = this.testExercises.find(x => x.name.toLowerCase() == exercise.name);
  ...
  editExercise() {
    let exercise = this.createExercise();
    let exerciseExists = this.testExercises.find(x => x.id == this.exerciseId);
    exerciseExists.name = exercise.name;
    exerciseExists.testType = exercise.testType;
    if (this.formEdit.value.minutes == undefined) { exerciseExists.minutes = 0 } else { exerciseExists.minutes = this.formEdit.value.minutes };
    if (this.formEdit.value.seconds == undefined) { exerciseExists.seconds = 0 } else { exerciseExists.seconds = this.formEdit.value.seconds };
    let i = this.testExercises.findIndex(x => x.id == this.exerciseId);
    console.log(i);
    this.testExercises[i] = exerciseExists;
